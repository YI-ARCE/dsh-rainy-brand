window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Loader entry id used by this package's shared configuration form. */
		...
		const zh = {
			title: "品牌",
			description: "侧栏字标与空白会话的大标题；修改即时生效，并保存到当前 profile 配置。",
			sidebarName: "侧栏名称",
			...
			cssTitle: "CSS 注入",
			cssDescription: "按选择器给页面注入自定义 CSS；改动即时生效，并保存到当前 profile 配置。",
			cssSelector: "选择器",
			...
			reset: "恢复默认",
			resetAll: "全部恢复默认",
			unavailable: "配置暂不可用：请确认品牌插件已启用并重启 dsh web。",
			refused: "配置未被接受，请检查当前配置后重试。",
		...
		const en = {
			title: "Brand",
			description: "The sidebar wordmark and the blank-session headline. Edits apply live and persist to the current profile configuration.",
			...
			cssDescription: "Inject custom CSS per selector. Edits apply live and persist to the current profile configuration.",
			...
			reset: "Reset",
			resetAll: "Reset all",
			unavailable: "Configuration unavailable: enable the brand plugin and restart dsh web.",
			refused: "The configuration change was not accepted. Check the current configuration and retry.",
		...
		function RainyBrandSection(props) {
			...
			const settleWrite = (operation, onFailure = () => {}) => {
			...
			const write = (field) => {
				...
				settleWrite(scope.set(field, value), () => {
					if (writtenRef.current[field] === value) delete writtenRef.current[field];
				});
			};
			...
			const writeRules = () => {
				...
				settleWrite(settle, () => {
					if (writtenRef.current.cssRules === rules) delete writtenRef.current.cssRules;
				});
			};
			...
			const clearAll = () => {
				...
				writtenRef.current = {};
				settleWrite(scope.mutate(ALL_FIELDS.map((field) => ({ op: "unset", path: [field] }))));
			};
